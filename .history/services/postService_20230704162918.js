...
exports.removeService = async (req, res, next) =>{
    const {postId} = req.params;
   const pos =  await  postModel.findById(postId);
   if (!user)
        return res.status(422).json({message:'not can find user'})
        clearImage(user.image);
        const removeUser =await postModel.findByIdAndRemove(postId);
             
      if(removeUser) return res.status(200).json({message:'the post removed ' , post})
          // post => res.status(200).json({message:'the post removed ' , post})
        res.status(402).json({message:'not can removed post' })
}
