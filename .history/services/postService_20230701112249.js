...
exports.uploadPostImage = async (req, res, next) => {
    if(req.file != undefined) {
     ...
     const fileName = Date.now() + '-' + file.originalname; 
      ...
      req.body.image = `http://localhost:8080/uploads/images/${fileName}`;
    }
next();
}
...
exports.getService = async (req, res, next) =>{
    ...
    const totalItems= await postModel.coun

    postModel
    .find()
    .limit(size)
    .then(posts => res.status(200).json({message:' sucessfuly posts' , posts, page, size}))
