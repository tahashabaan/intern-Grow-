...
exports.removeService = async (req, res, next) =>{
    const {postId} = req.params;
   const post =  await  postModel.fin(postId);
   if (!post)
        return res.status(422).json({message:'not can find user'})
        clearImage(user.image);
        const removePost =await postModel.findByIdAndRemove(postId);
             
      if(removePost) return res.status(200).json({message:'the post removed ' , post})
